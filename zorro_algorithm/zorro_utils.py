...
@tf.function
def get_mask_indices(X: tf.Tensor, V_s: tf.Tensor, F_s: tf.Tensor):
...
def fidelity(X: tf.Tensor,
    ...
    Smaller fidelity values mean less deviation from the original
    prediction. That is, lower means better.
    ...
    # Mark rows and columns given in the explanation as not-changable
    fixed_indices = get_mask_indices(X, V_s, F_s)
...
def compute_feature_fidelities(X: tf.Tensor,
    ...
    all_selected_features = tf.concat([selected_features, feature], axis=0)

    # TODO: 'tf.vectorized_map()' does not work when calling 'fidelity()'
    fidelity_value = fidelity(X, A, selected_nodes, all_selected_features, gnn)
    fidelity_value = tf.expand_dims(fidelity_value, axis=0)

    return tf.concat([tf.cast(feature, tf.float32), fidelity_value], axis=0)
...
def compute_node_fidelities(X: tf.Tensor,
    ...
    all_selected_nodes = tf.concat([selected_nodes, node], axis=0)

    # TODO: 'tf.vectorized_map()' does not work when calling 'fidelity()'
    fidelity_value = fidelity(X, A, all_selected_nodes, selected_features, gnn)
    fidelity_value = tf.expand_dims(fidelity_value, axis=0)

    return tf.concat([tf.cast(node, tf.float32), fidelity_value], axis=0)


@tf.function
def create_ranking(nodes: tf.Tensor,
                   features: tf.Tensor,
                   selected_nodes: tf.Tensor,
                   selected_features: tf.Tensor,
                   X: tf.Tensor,
    ...
    :param nodes: Variable nodes. For each, compute the fidelity value and rank them.
    :param features: Variable features. For each, compute the fidelity value and rank them.
    ...
    cff_partial = lambda x: compute_feature_fidelities(
        X, A, selected_features, x, selected_nodes, gnn
    )
    features_ranking = tf.map_fn(cff_partial, tf.cast(features, tf.float32))

    # Create ranking for fixed node and multiple features
    cnf_partial = lambda x: compute_node_fidelities(X, A, selected_nodes, x, selected_features, gnn)
